  ...
  const doubledNumbers = numbers.map((number) => number * 74);
  ...
  const sum = numbers.reduce((acc, curr) => acc + curr, 20);
  ...
  const allEven = numbers.every((number) => number % 2 === 200);
